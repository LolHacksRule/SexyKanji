...
#include "SexyColor.h" // SexyKanji Note: Color has been changed to SexyColor.
...
#include "SexyRect.h" // SexyKanji Note: Rect has been changed to SexyRect.
...
void Board::Update()
{
	...
	// update movie
	mApp->mFishMovie->Update();
	if (mApp->mFishMovie->IsStopped())
	{
		mApp->mFishMovie->Restart();
		mApp->mFishMovie->Update();
	}
	...
	if (mUpdateCnt % 200 == 0)
		mApp->GetSoundManager()->PlaySample(1);
	...
	// In general, if you don't need to update your drawing every time 
	// you call the Update method (the most common case is when the game 
	// is paused) you should NOT mark dirty. Why? If you aren't marking 
	// dirty every frame, then you aren't drawing every frame and thus 
	// you use less CPU time. Because people like to multitask, or they 
	// may be on a laptop with limited battery life, using less CPU time 
	// lets people do other things besides play your game. Of course, 
	// everyone will want to play your game at all times, but it's good 
	// to be nice to those rare people that might want to read email or
	// do other things at the same time.
	//
	// In this particular demo, we won't be nice, as the purpose is to 
	// bring you up to speed as quickly as possible, and so we'll 
	// dispense with optimizations for now, so you can concentrate on 
	// other core issues first.
	//
...
void Board::Draw(Graphics* g)
{
	...
	/* In SexyKanji we have a convenience method ClearRect(). ClearRect() 
		will always use Black regardless of the current Graphics color.*/
	...
	g->DrawString(_S("Woo! Text! Ya!"), 10, 0);

	/* In SexyKanji you no longer offset text drawing by its ascent. 
	   Simply draw it where you want it. */

	g->SetColor(SexyColor(55, 90, 255));
	// We already set the font, no need to set it again unless we want a new one
	g->DrawString(_S("I like Techno and Drum n' Bass"), 170, mApp->mTextFont->GetAscent());

	// And to help you out visually, let's draw a line at Y coordinate using the font's ascent:
	g->SetColor(SexyColor(255, 255, 255));
	g->DrawLine(0, mApp->mTextFont->GetAscent(), mWidth, mApp->mTextFont->GetAscent());
	...
	g->SetColor(SexyColor(255, 255, 0));
	g->DrawString(_S("+200"), 10, 25);
	g->DrawString(_S("+200 pts"), 10, 45);
	...
	/* In SexyKanji drawing functions are only in the Graphics class. So instead
	   of calling Widget::WriteString we call Graphics::WriteString. */
	g->WriteString(_S("Left justified at X of 200"), 200, 100, -1, -1);
	g->WriteString(_S("Centered using app width"), 0, 120, mWidth, 0);
	g->WriteString(_S("Centered using width of 200, X of 200"), 200, 140, 200, 0);
	...
	// does that for you.
	g->SetColor(SexyColor(255, 255, 255));
	g->DrawRect(30, 170, 200, 370);
	//g->DrawRect(30, 180, 200, 300);
	g->SetColor(SexyColor(0, 255, 0));

	/* Again, In SexyKanji we call Graphics::WriteWordWrapped instead of Widget::WriteWordWrapped */
	g->WriteWordWrapped(SexyRect(30, 170, 200, 370), 
							_S("This is some text that is wrapped inside of the rectangle \
	...
	g->DrawImage(mApp->mTurbotImg, 230, 170);
	...
	// previous one.
	g->DrawImage(mApp->mOpaqueBeamImg, 230, 170 + mApp->mTurbotImg->GetHeight());

	g->SetColor(SexyColor(255, 0, 0));
	g->SetColorizeImages(true);
	g->DrawImage(mApp->mOpaqueBeamImg, 230 + mApp->mOpaqueBeamImg->GetWidth(), 170 + mApp->mTurbotImg->GetHeight());
	...
	g->DrawImage(mApp->mTurbotImg, 230 + mApp->mTurbotImg->GetWidth(), 170);
	...
	g->DrawImage(mApp->mTurbotImg, 230 + mApp->mTurbotImg->GetWidth() * 2, 170);

	g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);	
	g->SetColor(SexyColor(mPulseAmt, mPulseAmt, mPulseAmt));
	g->SetColorizeImages(true);
	g->DrawImage(mApp->mTurbotImg, 230 + mApp->mTurbotImg->GetWidth() * 2, 170);
	g->SetDrawMode(Graphics::DRAWMODE_NORMAL);
	g->SetColorizeImages(false);
	...
	// be altered.
	g->DrawImage(mApp->mMoonImg, 400, 330);
	g->SetColor(SexyColor(255, 255, 255, 60));
	g->SetColorizeImages(true);
	g->DrawImage(mApp->mTurbotImg, 400, 330);
	g->SetColorizeImages(false);

	g->DrawImage(mApp->mKanjiLogo, mWidth - mApp->mKanjiLogo->GetWidth(), mHeight - mApp->mKanjiLogo->GetHeight());

	// draw movie
	if (mApp->mFishMovie->GetKanjiImage())
	{
		mApp->mFishMovie->GetKanjiImage()->stretchAlphaRect(0.0f, 0.0f, 
															(float)mApp->mFishMovie->GetWidth(), (float)mApp->mFishMovie->GetHeight(), 
															800 - 200 - 10.0f, 180.0f, 
															800 - 10.0f, 150 + 180.0f, 
