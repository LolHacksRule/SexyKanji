...
Board::Board(GameApp* theApp)
{
	...
	for (int i = 0; i < 3; i++)
	{
		...
		// to access:
		SexyString imageStringID = StringToSexyStringFast(StrFormat("IMAGE_BG%d", i));
		...
		mLayer[i].mY = (float)(mApp->GetHeight() - mLayer[i].mImage->GetHeight());
	}
}
...
void Board::KeyDown(EKeyboardLayout theKey)
{
	// If the user presses the right or left arrow keys, let's
	// scroll the layers:

	if (theKey == K_VK_RIGHT)
	{
		for (int i = 0; i < 3; i++)
		{
	...
	else if (theKey == K_VK_UP)
	{
		// increase the scaling of the board layer drawing
		mScaling += 0.01f;
	}
	else if (theKey == K_VK_DOWN)
	{
		// decrease the scaling of the board layer drawing
		mScaling -= 0.01f;
	}
}
...
{
	// This is an example of the SexyKanjiTransform, a Way to Transform the entire screen.
	SexyKanjiTransform aTrans;
	aTrans.Translate(-mWidth/2.0f, -mHeight/2.0f);
	aTrans.Scale(mScaling,mScaling);
	aTrans.Translate(mWidth/2.0f, mHeight/2.0f);
	...
	g->WriteWordWrapped(StrFormat(_S("%f"), mScaling),0,0);
	
	// We call this to avoid Future Drawing from being affected by the 
	// transforms.
	g->ClearTransforms();
}
...
void Board::AddedToManager(WidgetManager* theWidgetManager)
{
	...
	mButton->Resize(200, 5, IMAGE_BUTTON_NORMAL->GetWidth(), IMAGE_BUTTON_NORMAL->GetHeight());
	//mButton->Resize(56, 5, FONT_DEFAULT->StringWidth(mButton->mLabel) + 100, FONT_DEFAULT->GetHeight());
...
void Board::ButtonDepress(int theId)
{
	if (theId == Board::OPTIONS_BUTTON_ID)
	{
		...
		mApp->GetWidgetManager()->AddDialog(DemoDialog::DIALOG_ID, dlg);
	}
}
