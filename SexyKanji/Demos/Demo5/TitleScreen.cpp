...
void TitleScreen::Init(void)
{
	...
	mContinueLink->mColor = SexyColor(255, 255, 255);
	mContinueLink->mOverColor = SexyColor(0, 255, 0);
...
void TitleScreen::AddedToManager(WidgetManager *theWidgetManager)
{
	...
	int labelWidth = Sexy::FONT_DEFAULT->StringWidth(mContinueLink->mLabel);
	int labelHeight = Sexy::FONT_DEFAULT->GetHeight();
...
void TitleScreen::Draw(Graphics *g)
{
	...
	if (drawWidth > 0)
	{
		...
		g->DrawImage(IMAGE_LOADER_BAR, mWidth / 2 - loaderBarWidth / 2, 
					 400, SexyRect(0, 0, drawWidth, IMAGE_LOADER_BAR->GetHeight()));
	}
	...
	g->DrawImage(IMAGE_KANJI_LOGO, mWidth/2 - IMAGE_KANJI_LOGO->GetWidth()/2, mHeight/2 - IMAGE_KANJI_LOGO->GetHeight()/2);
}
...
void TitleScreen::ButtonDepress(int theId)
{
	if (theId == 1)
	{
		...
		mApp->GetWidgetManager()->RemoveWidget(this);
		mApp->GetWidgetManager()->RemoveWidget(mContinueLink);
