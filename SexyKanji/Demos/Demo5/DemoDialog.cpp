...
Dialog(IMAGE_DIALOG_BOX, IMAGE_DIALOG_BUTTON, DemoDialog::DIALOG_ID, true, StringToSexyStringFast(theHeader), StringToSexyStringFast(theBody), _S("CLOSE"), Dialog::BUTTONS_FOOTER)
{
	...
	int volume = gSexyAppBase->GetMusicVolume();
	mMusicVolumeSlider->SetValue(volume / 100.0f);
	...
	volume = gSexyAppBase->GetSfxVolume();
	mSfxVolumeSlider->SetValue(volume / 100.0f);
...
void DemoDialog::Draw(Graphics* g)
{
	...
	g->DrawString(_S("Full Screen:"), mFSCheckbox->mX - mX, mFSCheckbox->mY - mY - mFSCheckbox->mHeight);
	...
	g->SetColor(SexyColor(255, 0, 0, 128));	
...
void DemoDialog::Resize(int theX, int theY, int theWidth, int theHeight)
{
	...
	mFSCheckbox->Layout(LAY_SameLeft | LAY_Below, mRandomBtn, 0, 40, 0, 0);
...
void DemoDialog::ButtonDepress(int theId)
{
	...
	else if (theId == ID_YES)
	{
		...
		// Let's apply fullscreen mode settings first though.
		// We call SetWindowed. The first parameter is whether or not to run
		// windowed (false means fullscreen).

		gSexyAppBase->SetWindowed(!mFSCheckbox->mChecked);
