...
void LevelupEffect::Activate(LevelupStats ls)
{
	...
	gSexyAppBase->GetSoundManager()->PlaySample(SOUND_LEVEL_UP1);
}
...
void LevelupEffect::Update(float theFrac)
{
	...
	else if (mState == LevelupEffect::CURTAIN_IN)
	{
		...
		if ((mCurtainX += 25) >= gSexyAppBase->GetWidth() / 2)
		{
			mCurtainX = gSexyAppBase->GetWidth() / 2;
			mState = LevelupEffect::CURTAIN_OUT;
			gSexyAppBase->GetSoundManager()->PlaySample(SOUND_LEVEL_UP2);
		}
	}
	...
	else if (mState == LevelupEffect::COVERING_STATS)
	{
		...
		if (mStripSizeChange > 0)
		{
			...
			if ((mStripHeight += mStripSizeChange) >= gSexyAppBase->GetHeight())
			{
				...
				mStripHeight = 0;
				if ((mCoverWidth += STRIP_WIDTH) >= gSexyAppBase->GetWidth() / 2)
				{
...
void LevelupEffect::Draw(Graphics* g)
{
	...
	{		
		...
		g->SetColor( (SexyColor::HSLToRGB(mHue, 255, 128) & 0xFFFFFFFF) );
		for (int i = 0; i < mText.size(); i++)
		{
			BouncyChar* c = &mText[i];
	...
	else if (mState == LevelupEffect::CURTAIN_IN)
	{
		...
		g->FillRect(0, 0, mCurtainX, gSexyAppBase->GetHeight());
		g->FillRect(gSexyAppBase->GetWidth() - mCurtainX, 0, mCurtainX, gSexyAppBase->GetHeight());
	}
				...
				(mState == LevelupEffect::COVERING_STATS))
	{
		...
		g->DrawString(s, gSexyAppBase->GetWidth() / 2 - FONT_HUNGARR->StringWidth(s) / 2, y);
		...
		{
			y += 50;
			int third =  gSexyAppBase->GetWidth() / 3;
			...
			{
				if (y >= gSexyAppBase->GetHeight() - FONT_HUNGARR->GetHeight() * 2)
				{
					drawDotDotDot = true;
					break;
				}
				...
				g->SetColor(SexyColor(128, 255, 0));
				s = mStats.mPlanetsEaten[i+1];
				g->DrawString(s, third + (third / 2 - FONT_HUNGARR->StringWidth(s) / 2), y);

				g->SetColor(SexyColor(255, 128, 0));
		...
		g->DrawString(s, gSexyAppBase->GetWidth() / 2 - FONT_HUNGARR->StringWidth(s) / 2, gSexyAppBase->GetHeight() - 20);		
	}
	...
	if (mState == LevelupEffect::CURTAIN_OUT)
	{
		...
		g->SetColor(SexyColor(255, 0, 0, alpha));
		g->FillRect(0, 0, mCurtainX, gSexyAppBase->GetHeight());
		g->FillRect(gSexyAppBase->GetWidth() - mCurtainX, 0, mCurtainX, gSexyAppBase->GetHeight());
	}
	...
	if ((mState == LevelupEffect::COVERING_STATS) || (mState == LevelupEffect::FADE_OUT_STATS))
	{
		...
		g->SetColor( (SexyColor::HSLToRGB(mHue, 255, 128) & 0xFFFFFF) | (mFadeOutAlpha << 24) );
		g->FillRect(0, 0, mCoverWidth, gSexyAppBase->GetHeight());
		g->FillRect(gSexyAppBase->GetWidth() - mCoverWidth, 0, mCoverWidth, gSexyAppBase->GetHeight());

		if (mStripSizeChange > 0)
		{
			g->FillRect(mCoverWidth, 0, STRIP_WIDTH, mStripHeight);
			g->FillRect(gSexyAppBase->GetWidth() - mCoverWidth - STRIP_WIDTH,
						gSexyAppBase->GetHeight() - mStripHeight,
						STRIP_WIDTH,
						mStripHeight);
		}
		else
		{			
			g->FillRect(mCoverWidth, gSexyAppBase->GetHeight() - mStripHeight, STRIP_WIDTH, mStripHeight);
			g->FillRect(gSexyAppBase->GetWidth() - mCoverWidth - STRIP_WIDTH, 0, STRIP_WIDTH, mStripHeight);
		}
...
void LevelupEffect::DoneViewingStats()
{
	...
	mFadeOutAlpha = 255;
	mStartNextLevel = false;
	gSexyAppBase->GetSoundManager()->PlaySample(SOUND_LEVEL_UP3);
}
...
bool LevelupEffect::StartNextLevel()
{
	...
	{
		mStartNextLevel = false;
		gSexyAppBase->GetSoundManager()->PlaySample(SOUND_LEVEL_UP4);
		return true;
	}
