...
Board::Board(GameApp* theApp)
{
	...
	{
		Star s;
		s.mX = Rand() % mApp->GetWidth();
		s.mY = Rand() % mApp->GetHeight();

		int r = Rand() % 3;
		switch (r)
		{
			case 0: s.mSpeed = 0.5f; s.mColor = SexyColor(200, 200, 200, 128); break;
			case 1: s.mSpeed = 0.25f; s.mColor = SexyColor(100, 100, 100, 128); break;
			case 2: s.mSpeed = 0.1f; s.mColor = SexyColor(50, 50, 50, 128); break;
		}
...
void Board::Update()
{
	// If paused, return and don't markdirty, which prevents drawing
	...
	if (mGameOverEffect->IsActive())
	{
		...
		if (mGameOverEffect->CanInitFirstLevel())
		{
			...
			mApp->GetMusicManager()->PlayStream(0, false, 0.2f);
			InitLevel(1);
		}
	}
...
void Board::UpdateF(float theFrac)
{
	...
	if ((!mLevelupEffect->HidePlanets() || (mPauseLevel > 0)) && !mGameOverEffect->IsActive())
	{
		...
		for (int i = 0; i < mPlanets.size(); i++)
		{
			...
			else
			{
				...
				{
					if (++p->mExplodeFrame >= IMAGE_BOMB_RADIAL_DEATH->GetColumns())
					{
	...
	for (i = 0; i < mParticles.size(); i++)
	{
		...
		if (p->mTimer % 6 == 0)
		{
			if (++p->mFrame >= IMAGE_PARTICLE_LIGHTNING->GetColumns())
			{
...
void Board::MoveLines(float theFrac)
{
	...
	{
		int modVal = 2; //gSexyAppBase->Is3DAccelerated() ? 2 : 4;
	...
	if ((mMovingLine1.mDone && mMovingLine2.mDone) ||
		...
		mApp->GetSoundManager()->StopLoopingSample(SOUND_BEAM_MOVING);
}
...
void Board::Draw(Graphics* g)
{
	...
	int incAmt = 1; //gSexyAppBase->Is3DAccelerated() ? 1 : 2;
...
void Board::DrawGrid(Graphics* g)
{
	...
	// why it's useful.
	if (mFlashCount > 0)
		g->SetColor(SexyColor::HSLToRGB(mBorderHue, 255, 128) & 0xFFFFFFFF);
	else
		g->SetColor(SexyColor(255, 0, 0, 64));
	...
	for (int y = 0; y < GRID_HEIGHT; y++)
	{
		...
		for (int x = 0; x < GRID_WIDTH; x++)
		{
			...
			if (state == GRID_FILLING)
			{
				...
				SexyFRect* fr = &mGridState[y][x].mFillRect;
				SexyRect normalRect;

				if (mFillDirection == FILL_RIGHT)
					normalRect = SexyRect(fr->mX + fr->mWidth, drawY, GRID_PIX_SIZE - fr->mWidth + 1, GRID_PIX_SIZE);
				else if (mFillDirection == FILL_LEFT)
					normalRect = SexyRect(drawX, drawY, GRID_PIX_SIZE - fr->mWidth, GRID_PIX_SIZE);
				else if (mFillDirection == FILL_UP)
					normalRect = SexyRect(drawX, drawY, GRID_PIX_SIZE, GRID_PIX_SIZE - fr->mHeight);
				else
					normalRect = SexyRect(drawX, fr->mY + fr->mHeight, GRID_PIX_SIZE, GRID_PIX_SIZE - fr->mHeight + 1);


				if ((normalRect.mWidth > 0) && (normalRect.mHeight > 0))
				{					
					g->SetColor(SexyColor(255, 255, 0, startBright && (x % 2 == 0) ? 128 : 64));
					g->FillRect(normalRect);
					g->SetColor(SexyColor(0, 0, 0));
					//g->DrawRect(normalRect);
				}

				g->SetColor(SexyColor(255, 255, 0, 100));
				FillRectF(g, *fr);
			}
			else if (state == GRID_NORMAL)
			{
				...
				g->SetColor(SexyColor(255, 255, 0, startBright && (x % 3 == 0) ? 64 : 32));	
...
void Board::DrawUI(Graphics* g)
{
	...
	g->SetColor(SexyColor(255, 255, 255, 128));
	g->DrawString(s, rightX - strWidth, 0);
	g->SetColor(SexyColor(255, 0, 0, 200));
	g->DrawString(StrFormat(_S("%d"), mNumPlanetsEaten), rightX - 5, 0);

	s = _S("POPULATION CONSUMED: ");
	g->SetColor(SexyColor(255, 255, 255, 128));
	g->DrawString(s, 5, height);
	g->SetColor(SexyColor(255, 0, 0, 200));
	g->DrawString(CommaSeperate(mPopulationEaten), rightX - 5, height);	

	s = _S("SCORE: ");
	strWidth = FONT_HUNGARR->StringWidth(s);
	g->SetColor(SexyColor(255, 255, 255, 128));
	g->DrawString(s, rightX - strWidth, height * 2);
	g->SetColor(SexyColor(255, 255, 0, 200));
	g->DrawString(StrFormat(_S("%s"), CommaSeperate(mScore).c_str()), rightX - 5, height * 2);
	...
	g->DrawString(s, x, 0);
	g->SetColor(SexyColor(255, 0, 0, 200));
	g->DrawString(StrFormat(_S("%d%%"), mPercentComplete), x + FONT_HUNGARR->StringWidth(s), 0);

	s = _S("LIVES: ");
	g->SetColor(SexyColor(255, 255, 255, 128));
	g->DrawString(s, x, height);
	strWidth = FONT_HUNGARR->StringWidth(s);
	g->DrawImage(IMAGE_HUNGARR_SMALL, strWidth + x, ascent + IMAGE_HUNGARR_SMALL->GetHeight() / 3);
	g->SetColor(SexyColor(255, 0, 0, 200));
	g->DrawString(StrFormat(_S("x%d"), mLives), x + 10 + strWidth + IMAGE_HUNGARR_SMALL->GetWidth(), height);	

	s = _S("LEVEL: ");
	g->SetColor(SexyColor(255, 255, 255, 128));
	g->DrawString(s, x, height * 2);
	g->SetColor(SexyColor(255, 255, 0, 200));
	g->DrawString(StrFormat(_S("%d"), mLevel), x + FONT_HUNGARR->StringWidth(s), height * 2);

	for (int i = 0; i < mBonusText.size(); i++)
	{
		BonusText* bt = &mBonusText[i];
		g->SetColor( (SexyColor::HSLToRGB(bt->mHue, 255, 128) & 0xFFFFFF) | (bt->mAlpha << 24) );
...
void Board::Beam1DrawHelper(Graphics* g)
{
	...
	if (mMovingLine1.mIsVertical)
	{
		if (true) //(gSexyAppBase->Is3DAccelerated())
		{
			g->DrawImageF(IMAGE_HUNGARR_BEAM_UP, mMovingLine1.mX - 8, mMovingLine1.mY, 
				SexyRect(0, 0, IMAGE_HUNGARR_BEAM_UP->GetWidth(), mMovingLine1.mHeight));
		}
		else
		{
	...
	else
	{
		if (true) //(gSexyAppBase->Is3DAccelerated())
		{
			g->DrawImageF(IMAGE_HUNGARR_BEAM_LEFT, mMovingLine1.mX, mMovingLine1.mY - 8,
				SexyRect(0, 0, mMovingLine1.mWidth, IMAGE_HUNGARR_BEAM_LEFT->GetHeight()));
		}
		else
		{
			g->DrawImageF(IMAGE_HUNGARR_BEAM_LEFT, mMovingLine1.mX, mMovingLine1.mY - 8,
	...
	if (mMovingLine2.mIsVertical)
	{
		if (true) //(gSexyAppBase->Is3DAccelerated())
		{
			g->DrawImageF(IMAGE_HUNGARR_BEAM_DOWN, mMovingLine2.mX - 8, mMovingLine2.mY - 1, 
				SexyRect(0, IMAGE_HUNGARR_BEAM_DOWN->GetHeight() - mMovingLine2.mHeight, 
	...
	else
	{
		if (true) //(gSexyAppBase->Is3DAccelerated())
		{
			g->DrawImageF(IMAGE_HUNGARR_BEAM_RIGHT, mMovingLine2.mX - 1, mMovingLine2.mY - 8,
				SexyRect(IMAGE_HUNGARR_BEAM_RIGHT->GetWidth() - mMovingLine2.mWidth, 0, 
...
{		
	...
	{		
		...
		if (!mMovingLine1.mDone)
		{
			...
			g->SetColor(SexyColor(mBeamPulseVal, mBeamPulseVal, mBeamPulseVal));			
			Beam1DrawHelper(g);
	...
	{
		Particle* p = &mParticles[i];		
		g->DrawImageCel(IMAGE_PARTICLE_LIGHTNING, p->mX, p->mY, p->mFrame, 0);		
	}
	g->SetDrawMode(Graphics::DRAWMODE_NORMAL);
}
...
void Board::DrawPlanets(Graphics* g)
{
	...
	for (int i = 0; i < mPlanets.size(); i++)
	{
		...
		{
			SexyRect r = SexyRect(p->mImgCol * w, 0, w, IMAGE_PLANETS->GetCelHeight());
...
void Board::DrawHungarrVertBeamsHelper(Graphics* g)
{
	...
	if (true) //(gSexyAppBase->Is3DAccelerated())
	{
		g->DrawImageF(IMAGE_HUNGARR_BEAM_UP, mLine1X, mLine1Y, 
			SexyRect(0, 0, IMAGE_HUNGARR_BEAM_UP->GetWidth(), h));

		g->DrawImageF(IMAGE_HUNGARR_BEAM_DOWN, mLine2X, mLine2Y, 
			SexyRect(0, IMAGE_HUNGARR_BEAM_DOWN->GetHeight() - h, IMAGE_HUNGARR_BEAM_DOWN->GetWidth(), h));

		g->SetDrawMode(Graphics::DRAWMODE_ADDITIVE);
		g->SetColorizeImages(true);

		g->SetColor(SexyColor(255, 255, 255, mBeamPulseVal));
		g->DrawImageF(IMAGE_HUNGARR_BEAM_UP, mLine1X, mLine1Y, 
			SexyRect(0, 0, IMAGE_HUNGARR_BEAM_UP->GetWidth(), h));
...
void Board::DrawHungarrHorizBeamsHelper(Graphics* g)
{
	...
	if (true) //(gSexyAppBase->Is3DAccelerated())
	{
		g->DrawImageF(IMAGE_HUNGARR_BEAM_LEFT, mLine1X, mLine1Y, 
			SexyRect(0, 0, w, IMAGE_HUNGARR_BEAM_LEFT->GetHeight()));

		g->DrawImageF(IMAGE_HUNGARR_BEAM_RIGHT, mLine2X, mLine2Y, 
			SexyRect(IMAGE_HUNGARR_BEAM_RIGHT->GetWidth() - w, 0, w, IMAGE_HUNGARR_BEAM_RIGHT->GetHeight()));
...
void Board::DrawHungarr(Graphics* g)
{
	...
	bool is3d = true; //gSexyAppBase->Is3DAccelerated();
...
{	
	...
	mOptionsBtn->SetColor(ButtonWidget::COLOR_LABEL, SexyColor::White);
	mOptionsBtn->SetColor(ButtonWidget::COLOR_LABEL_HILITE, SexyColor::White);	
	...
	int y = FONT_HUNGARR->GetHeight() * 3 - 20;
	mOptionsBtn->Resize(gSexyAppBase->GetWidth() - IMAGE_BUTTON_NORMAL->GetWidth() - 10, y, 
...
void Board::ButtonDepress(int theId)
{
	...
	if (theId == mOptionsBtn->mId)
	{
		...
		mApp->GetSoundManager()->PlaySample(SOUND_BUTTON);
		Pause(true);
		OptionsDialog* od = new OptionsDialog(this);
		od->Resize(mWidth / 2 - 200, mHeight / 2 - 175, 400, 350);
		mApp->GetWidgetManager()->AddDialog(OptionsDialog::DIALOG_ID, od);
	}
}
...
{		
	...
	else if ((theClickCount > 0) && mMovingLine1.mDone && mMovingLine2.mDone && !mFilling)
	{
		...
		SexyFRect hungarrRect = SexyFRect(mHungarrX, mHungarrY, IMAGE_HUNGARR_HORIZ->GetWidth(), IMAGE_HUNGARR_HORIZ->GetHeight());
		for (int i = 0; i < mPlanets.size(); i++)
		{
			Planet* p = &mPlanets[i];
			SexyFRect planetRect = SexyFRect(p->mX, p->mY, IMAGE_PLANETS->GetCelWidth(), IMAGE_PLANETS->GetCelHeight());
		...
		mApp->GetSoundManager()->LoopSample(SOUND_BEAM_MOVING);
				...
				switch (mFillDirection)
				{
					case FILL_RIGHT: 
						mGridState[y][x].mFillRect = 
							SexyFRect(GetColPix(x), GetRowPix(y), 0, GRID_PIX_SIZE); 
						break;

					case FILL_LEFT:
						mGridState[y][x].mFillRect = 
							SexyFRect(GetColPix(x + 1), GetRowPix(y), 0, GRID_PIX_SIZE); 
						break;

					case FILL_UP:
						mGridState[y][x].mFillRect = 
							SexyFRect(GetColPix(x), GetRowPix(y + 1), GRID_PIX_SIZE, 0); 
						break;

					case FILL_DOWN:
						mGridState[y][x].mFillRect = 
							SexyFRect(GetColPix(x), GetRowPix(y), GRID_PIX_SIZE, 0); 
						break;
				}
			}
		}
...
//////////////////////////////////////////////////////////////////////////
//////////////////////////////////////////////////////////////////////////
void Board::FillRectF(Graphics* g, SexyFRect fr)
{
	SexyRect r = SexyRect((int)fr.mX, (int)fr.mY, (int)fr.mWidth, (int)fr.mHeight);
	g->FillRect(r);
}
...
void Board::InitLevel(int level)
{
	...
	for (i = 0; i < GRID_HEIGHT; i++)
	{
		for (int x = 0; x < GRID_WIDTH; x++)
		{
			mGridState[i][x].mFillRect = SexyFRect(x * GRID_PIX_SIZE, GRID_START_Y + i * GRID_PIX_SIZE, 0, 0);
	...
	for (i = 0; i < numPlanets; i++)
	{
		...
		p.mImgCol = Rand() % IMAGE_PLANETS->GetColumns();
...
void Board::CheckPlanetBeamCollision(Planet* p)
{
	...
	SexyFRect pr = SexyFRect(p->mX + 4, p->mY + 4, 11, 11);

	SexyFRect beam1Rect;

	if (!mMovingLine1.mIsVertical)
		beam1Rect = SexyFRect(mMovingLine1.mX + 7, mMovingLine1.mY + 12, mMovingLine1.mWidth - 12, 8);
	else
		beam1Rect = SexyFRect(mMovingLine1.mX + 12, mMovingLine1.mY + 10, 9, mMovingLine1.mHeight);
	...
	if (!mMovingLine2.mIsVertical)
		beam2Rect = SexyFRect(mMovingLine2.mX, mMovingLine2.mY + 12, mMovingLine2.mWidth - 7, 8);
	else
		beam2Rect = SexyFRect(mMovingLine2.mX + 12, mMovingLine2.mY, 9, mMovingLine2.mHeight);
...
{	
	...
	if (--mLives <= 0)
	{
		...
		// Fade out the music
		mApp->GetMusicManager()->FadeOutStream(0, 0.4f);
...
void Board::OptionsDialogDone()
{
	...
	mApp->GetWidgetManager()->SetFocus(this);
}
