...
void GameOverEffect::Init()
{
	...
	for (int i = 0; i < gSexyAppBase->GetWidth(); i++)
...
void GameOverEffect::Update(void)
{
	...
	if (!mFadeOut)
	{
		...
		if ((mState == RED_FADE_IN) || (mState == RED_HOLD))
			if (mUpdateCnt % 40 == 0)
				gSexyAppBase->GetSoundManager()->PlaySample(SOUND_EXPLOSION);
	...
	else if (mState == GameOverEffect::RED_HOLD)
	{
		...
		if (--mRedHoldCount <= 0)
		{
			...
			mUpdateCnt = 0;
			gSexyAppBase->GetSoundManager()->PlaySample(SOUND_GAME_OVER_TEXT);
		}
		else
			PulseRed();
	}
	...
	else if (mState == GameOverEffect::FADING_LETTERS)
	{
		...
		if (done)
		{
			...
			gSexyAppBase->GetSoundManager()->PlaySample(SOUND_GAME_OVER_STATS);
		}
	...
	else if ((mState == GameOverEffect::DRIP_OUT) && !mFadeOut)
	{
		...
		for (int i = 0; i < mLines.size(); i++)
		{
			...
			else if (!dl->mDripUp)
			{
				...
				if (dl->mHeight >= gSexyAppBase->GetHeight())
				{
					dl->mHeight = 0;
					dl->mSpeed = 10;
					dl->mDripUp = true;
			...
			else if (dl->mDripUp && (dl->mHeight < gSexyAppBase->GetHeight()))
			{
				dl->mHeight += dl->mSpeed;
				if (dl->mHeight >= gSexyAppBase->GetHeight())
					dl->mHeight = gSexyAppBase->GetHeight();
				else
					alldone = false;
			}
...
void GameOverEffect::Draw(Graphics* g)
{
	...
	if (!mFadeOut)
	{
		...
		{
			Explosion* e = &mExplosion[i];
			g->DrawImageCel(IMAGE_ATOMIC_EXPLOSION, e->mX, e->mY, e->mFrame, 0);
		}
	}

	if ((mState == GameOverEffect::RED_FADE_IN) || (mState == GameOverEffect::RED_HOLD))
	{
		g->SetColor(SexyColor(mRed, 0, 0, mAlpha));
		g->FillRect(0, 0, gSexyAppBase->GetWidth(), gSexyAppBase->GetHeight());
	}
	else if ((mState == GameOverEffect::SHOWING_LETTERS) || (mState == GameOverEffect::FADING_LETTERS))
	{
		g->SetFont(FONT_HUNGARR);
		int x = gSexyAppBase->GetWidth() / 2 - FONT_HUNGARR->StringWidth(_S("GAME OVER")) / 2;
		int y = gSexyAppBase->GetHeight() / 2;
		...
		for (int i = 0; i < mText.size(); i++)
		{
			...
			if (l->mRed > 0)
			{
				g->SetColor(SexyColor(l->mRed, 0, 0));
	...
	{		
		g->DrawImage(IMAGE_HUNGARR_LOGO, gSexyAppBase->GetWidth() / 2 - IMAGE_HUNGARR_LOGO->GetWidth() / 2, 10);

		g->SetFont(FONT_HUNGARR);
		g->SetColor(SexyColor::White);
		int rightX = gSexyAppBase->GetWidth() / 2;
		int y = 100;
		SexyString s;
		int strWidth;
	...
	else if (mState == GameOverEffect::DRIP_OUT)
	{
		if (!mFadeOut)
		{
			...
			SexyColor hue = SexyColor::HSLToRGB(mHue, 255, 128) & 0xFFFFFFFF;
			for (int i = 0; i < mLines.size(); i++)
			{
				DrippyLine* dl = &mLines[i];
				if (!dl->mDripUp && (dl->mDelay == 0))
				{
					...
					g->DrawRect(SexyRect(dl->mX, 0, 1, dl->mHeight - 1));
					g->SetColor(hue);
					g->DrawRect(SexyRect(dl->mX, dl->mHeight, 1, 1));
				}
				else if (dl->mDelay == 0)
				{
					...
					{
						g->SetColor(SexyColor(0, 0, 0));
						g->DrawRect(SexyRect(dl->mX, 0, 1, gSexyAppBase->GetHeight() - dl->mHeight));
					}

					g->SetColor(hue);
					g->DrawRect(SexyRect(dl->mX, gSexyAppBase->GetHeight() - dl->mHeight, 1, dl->mHeight));
				}
			}
		}
		else
		{
			...
			SexyColor hue = (SexyColor::HSLToRGB(mHue, 255, 128) & 0xFFFFFF) | (mAlpha << 24);
			g->SetColor(hue);
			g->FillRect(0, 0, gSexyAppBase->GetWidth(), gSexyAppBase->GetHeight());
		}
	}
...
{
	mState = DRIP_OUT;
	gSexyAppBase->GetSoundManager()->PlaySample(SOUND_GAME_OVER_CLICK);
	gSexyAppBase->GetSoundManager()->PlaySample(SOUND_GAME_OVER_RESTART);
}