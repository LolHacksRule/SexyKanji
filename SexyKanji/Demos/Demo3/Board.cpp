...
void Board::Update()
{
	...
	if (mUpdateCnt % 5 == 0)
	{
		...
		// to 0.
		if (++mAnimFrame >= mApp->mLightningImg->GetRows())
			mAnimFrame = 0;
	}
...
void Board::Draw(Graphics* g)
{
	...
	/* In SexyKanji this call has been changed to SetLinearBlend */
	...
	/* In SexyKanji this is no longer true. There is no software mode in SexyKanji. 
	   However, using SetLinearBlend can still affect performance. */
	...
	/* In SexyKanji we are always in 3D Mode. */
	...
	// SetLinearBlend call to set whether you want the nice, slow, smooth scaling, or the quick
	// and efficient exact scaling. In 3D mode, this is just as fast as a normal stretched draw.
	/* In SexyKanji we are always in 3D Mode. */
	g->DrawImageMirror(mApp->mTurbotImg, 
						SexyRect(200, 275, mApp->mTurbotImg->GetWidth() * 2, mApp->mTurbotImg->GetHeight() * 2),
						SexyRect(0, 0, mApp->mTurbotImg->GetWidth(), mApp->mTurbotImg->GetHeight()));
	...
	/* In SexyKanji you should always pass column and row. */
	g->DrawImageCel(mApp->mLightningImg, 0, 540, 0, mAnimFrame);
	...
	//SexyRect(0, 540, mApp->mLightningImg->GetWidth(), mApp->mLightningImg->GetCelHeight()),
	//SexyRect(0, mApp->mLightningImg->GetCelHeight() * mAnimFrame, mApp->mLightningImg->GetWidth(), mApp->mLightningImg->GetCelHeight()));
	//
	...
	/* In SexyKanji drawing functions are only in the Graphics class. So instead
	   of calling Widget::WriteWordWrapped we call Graphics::WriteWordWrapped. */
	g->WriteWordWrapped(SexyRect(630, 40, mWidth - 630, 300), buttonStr, -1, -1);

	g->DrawImage(mApp->mKanjiLogo, mWidth - mApp->mKanjiLogo->GetWidth(), mHeight - mApp->mKanjiLogo->GetHeight()*2);
}
...
void Board::AddedToManager(WidgetManager* theWidgetManager)
{
	...
	mButton->SetColor(ButtonWidget::COLOR_LABEL, SexyColor(0, 0, 0));
	mButton->SetColor(ButtonWidget::COLOR_LABEL_HILITE, SexyColor(0, 255, 0));
