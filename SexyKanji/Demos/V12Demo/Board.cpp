...
Board::Board(V12DemoApp* theApp)
{
	...
	mDialogButton->Resize(mApp->GetWidth() - 20 - w, 10, w + 10, 50);
	...
	mRect = SexyRect(mApp->GetWidth() / 2 - 1, mApp->GetHeight() / 2 - 1, 2, 2);
	mExpanding = true;
	mMsgX = Rand() % (mApp->GetWidth() - 100);
	mMsgY = Rand() % (mApp->GetHeight() - 100);
	mLostFocus = false;
...
Board::~Board()
{
	...
	if (mDemoWidget != NULL)
		mApp->GetWidgetManager()->RemoveWidget(mDemoWidget);
...
void Board::Update()
{
	...
	{		
		...
		if (mExpanding)
		{
			...
			if (mRect.mWidth >= mApp->GetWidth())
			{
				mRect.mWidth = mApp->GetWidth();
				mExpanding = false;
			}

			if (mRect.mHeight >= mApp->GetHeight())
			{
				mRect.mHeight = mApp->GetHeight();
				mExpanding = false;
			}
		}
		else
		...
		mRect.mX = mApp->GetWidth() / 2 - mRect.mWidth / 2;
		mRect.mY = mApp->GetHeight() / 2 - mRect.mHeight / 2;
	}


	// In SexyKanji we don't support direct control of the Update loop. So in 
	// this demo we have moved the relevant code from the ButtonDepress method 
	// to here. Notice that we no longer use a while loop, since we are in an update loop.
...
void Board::Draw(Graphics* g)
{
	...
	if (!mLostFocus)
	{
		...
		g->SetColor(SexyColor(mUpdateCnt % 128, mUpdateCnt % 255, mUpdateCnt % 64));
		g->DrawImage(IMAGE_HUNGARR_LOGO, 10, 100);
	}
...
void Board::DrawOverlay(Graphics* g)
{
	...
	g->SetColor(SexyColor(0, 0, 255, 175));
	g->FillRect(0, 0, mWidth, mHeight);
	...
	//if (gSexyAppBase->Is3DAccelerated())
		g->DrawImageTransformF(IMAGE_HUNGARR_LOGO, t, (float)(mUpdateCnt % (mApp->GetWidth() + 340)), 200.0f);
	//else
...
void Board::ButtonDepress(int id)
{
	if (id == mDemoButton->mId)
	{
		...
		mApp->GetWidgetManager()->AddWidget(mDemoWidget);
		...
		mApp->GetWidgetManager()->AddBaseModal(mDemoWidget, flags);
	}
	else if (id == mDialogButton->mId)
	{
		...
		Dialog* d = mApp->GetWidgetManager()->DoDialog(100, true, _S("Fun Dialog"), _S("Line 1\nLine 2\nLine 3"), _S("Close!"), Dialog::BUTTONS_FOOTER);
	...
	else if (id == mCurtainButton->mId)
	{
		...
		// In SexyKanji we don't support direct control of the Update loop. So in 
		// this demo you would need to move the relevant code to the Update method 
