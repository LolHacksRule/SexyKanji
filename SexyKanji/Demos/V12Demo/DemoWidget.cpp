...
DemoWidget::DemoWidget()
{
	...
	int w = 200, h = 200;
	Resize(gSexyAppBase->GetWidth() / 2 - w / 2, gSexyAppBase->GetHeight() / 2 - h / 2, w, h);
...
void DemoWidget::ButtonDepress(int id)
{
	if (id == mMoveButton->mId)
	{
		...
		Move(Rand() % (gSexyAppBase->GetWidth() - mWidth), Rand() % (gSexyAppBase->GetHeight() - mHeight));
	}
	else if (id == mCloseButton->mId)
	{
		// We'll remove ourselves when the close button is pressed.
		gSexyAppBase->GetWidgetManager()->RemoveWidget(this);
	}
}
...
{
	g->SetColor(SexyColor(0, 255, 0, 200));
