...
// \details 	<b>Description:</b> 
//************************************/
SexyRect WidgetContainer::GetRect()
{
	return SexyRect(mX, mY, mWidth, mHeight);	
}
...
/*!***********************************
// @return    	Sexy::SexyPoint
//
// \brief		
// \details 	<b>Description:</b> 
//************************************/
SexyPoint WidgetContainer::GetAbsPos() // relative to top level
{
	if (mParent==NULL)
		return SexyPoint(mX,mY);
	else
		return SexyPoint(mX,mY) + mParent->GetAbsPos();
}
		...
		for (;;)
		{
			...
			if (aWidget->mVisible)
			{
				if ((!aWidget->mHasTransparencies) && (!aWidget->mHasAlpha))
				{
					...
					SexyRect aRect = SexyRect(theWidget->mX,theWidget->mY,theWidget->mWidth,theWidget->mHeight).Intersection(SexyRect(0,0,mWidth,mHeight)); 
