...
{		
	...
	for (int i = 0; i < (int)theString.length(); i++)
	{
		if (theString[i] == 0x100)
		{
			...
			aCurString = _S("");
			if (useColors)
				g->SetColor(SexyColor(theString[i+1], theString[i+2], theString[i+3]));
			i += 3;
		}
		else
...
SexyColor TextWidget::GetLastColor(const SexyString& theString)
{
	int anIdx = theString.rfind((char)0xFF);
	if (anIdx < 0)
		return SexyColor(0, 0, 0);
	
	return SexyColor(theString[anIdx+1], theString[anIdx+2], theString[anIdx+3]);
}
	...
	else
	{
		...
		while (aCurPos < (int)theLine.length())
		{
			...
			if (GetColorStringWidth(aNewString) > mWidth-8)
			{
				...
				SexyColor aColor = GetLastColor(aCurString);
...
void TextWidget::Draw(Graphics* g)
{
	...
	int aLastLine = K_MIN((int) mPhysicalLines.size()-1, (int) mPosition + (int) mPageSize + 1);
...
void TextWidget::KeyDown(EKeyboardLayout theKey)
{			
	/*if (theKey == 3)
	{
		// Control-C		
